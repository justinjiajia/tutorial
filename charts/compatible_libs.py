...
code = """
# to run this snippet on your computer, you need to first install the library using pip install plotly
import plotly.express as px
...
code = """
# to run this snippet on your computer, you need to first install the library using pip install altair
import altair as alt
...
c = (alt.Chart(gapminder)
     .mark_circle()
     .encode(
         alt.X("gdpPercap", title='GDP Per Capita').scale(type="log", domain=[100, 100000]), 
         alt.Y("lifeExp", title='Life Expectancy').scale(domain=[25, 90], bins=[30, 40, 50, 60, 70, 80, 90]), 
         alt.Size('pop').scale(rangeMax=3000).legend(None), 
         alt.Color('continent', title="Continent", sort=["Asia", "Africa", "Americas", "Oceania", "Europe"])
...
with st.container(border=True):
    ...
    alt.Chart(gapminder)
    .mark_circle()
    .encode(alt.X("gdpPercap", title='GDP Per Capita').scale(type="log", domain=[100, 100000]), 
            alt.Y("lifeExp", title='Life Expectancy').scale(domain=[25, 90], bins=[30, 40, 50, 60, 70, 80, 90]), 
            alt.Size('pop').scale(rangeMax=3000).legend(None), 
            alt.Color('continent', title="Continent", sort=["Asia", "Africa", "Americas", "Oceania", "Europe"])
               .scale(scheme="set1").legend(orient='top-left'),
